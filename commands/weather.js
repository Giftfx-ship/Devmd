...
module.exports = async function weatherCommand(sock, chatId, city, message) {
  try {
    ...
    const apiKey = process.env.OPENWEATHER_KEY || '4902c0f2550f58298ad4146a92b65e10';
    ...
    if (error.response && error.response.data && error.response.data.message) {
      await sock.sendMessage(chatId, { text: `❌ ${error.response.data.message}` }, { quoted: message });
    } else {
      await sock.sendMessage(chatId, { text: '❌ Could not fetch weather. Try again later.' }, { quoted: message });
    }
  }
};
