...
import settings from "../settings.js"; // for botName & channel
import { downloadContentFromMessage } from "@whiskeysockets/baileys";
import { writeFile } from "fs/promises";
import { isAdmin } from "../lib/isadmin.js"; // Import your isAdmin helper
...
export async function handleAntideleteCommand(sock, chatId, message, match) {
  ...
  if (!message.key.fromMe) {
    return sock.sendMessage(
      chatId,
      { text: "*Only the bot owner can toggle antidelete.*" },
      { quoted: message }
    );
  }
...
export async function handleMessageRevocation(sock, revocationMessage) {
  try {
    ...
    const groupName = original.group
      ? (await sock.groupMetadata(original.group)).subject
      : "";
